...
import { MentorGroupForumTopic as MentorGroupForumTopicType } from '@/app/type/types';
import { Card, Button, Badge } from '@/app/ui';
import axios from 'axios';

interface MentorGroupForumTopicProps {
    topicId: number;
    onBack: () => void;
}

const MentorGroupForumTopicView: React.FC<MentorGroupForumTopicProps> = ({ topicId, onBack }) => {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
    const [topic, setTopic] = useState<MentorGroupForumTopicType | null>(null);
    ...
    const fetchTopic = async () => {
        ...
        try {
            ...
            const response = await axios.get(
                `${apiUrl}/mentorGroupForum/topic/${topicId}`,
                {
    ...
    const handleSubmitPost = async (e: React.FormEvent) => {
        ...
        try {
            setSubmitting(true);
            await axios.post(
                `${apiUrl}/mentorGroupForum/topic/${topicId}/posts`,
                { content: newPost },
                {
                    headers: { Authorization: `Bearer ${token}` }
                }
...
export default MentorGroupForumTopicView;