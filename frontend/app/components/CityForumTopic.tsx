...
import { CityForumTopic as CityForumTopicType } from '@/app/type/types';
import { Card, Button, Badge } from '@/app/ui';
import axios from 'axios';

interface CityForumTopicProps {
    topicId: number;
    onBack: () => void;
}

const CityForumTopicView: React.FC<CityForumTopicProps> = ({ topicId, onBack }) => {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
    const [topic, setTopic] = useState<CityForumTopicType | null>(null);
    ...
    const fetchTopic = async () => {
        try {
            setLoading(true);
            const response = await axios.get(`${apiUrl}/cityForum/topic/${topicId}`);
    ...
    const handleSubmitPost = async (e: React.FormEvent) => {
        ...
        try {
            setSubmitting(true);
            await axios.post(
                `${apiUrl}/cityForum/topic/${topicId}/posts`,
                { content: newPost },
                {
...
export default CityForumTopicView;